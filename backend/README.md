...
│   ├── models.py                  # ORM models (Invoice, AuditLog)
│   ├── schemas.py                 # Pydantic request/response schemas
│   ├── routers/                   # API route controllers
│   │   ├── auth.py                # API key authentication dependency
│   │   ├── invoices.py            # Invoice ingestion, review, update, export endpoints
│   │   └── stats.py               # Dashboard statistics and health check endpoints
│   ├── services/                  # Business logic layer
│   │   ├── processor.py           # Core pipeline orchestration & response mapping
│   │   ├── doc_extraction_engine.py # pdfplumber text extraction & PDF classification
│   │   ├── ai_parser.py           # LangGraph state machine & Gemini 2.5 Flash parsing
│   │   ├── validator.py           # GST compliance validation & confidence scoring
│   │   ├── reviewer.py            # Human-in-the-Loop review decision logic
│   │   ├── exporter.py            # CSV and JSON export formatters
│   │   ├── tally_exporter.py      # Tally-compatible XML voucher generation
│   │   └── erpnext_exporter.py    # ERPNext Purchase Invoice API push
